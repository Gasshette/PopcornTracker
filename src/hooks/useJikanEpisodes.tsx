...
function findBestMatch(
  ...
  for (const anime of jikanResults) {
    ...
    DEBUGS.JIKAN && console.log(`[DEBUG] Candidate (MAL ID: ${anime.mal_id}):`, {
      title: anime.title,
  ...
  DEBUGS.JIKAN && console.warn(`[DEBUG] Best match overall (Score=${bestScore.toFixed(3)}):`, bestMatch);
...
function useJikanMatch(media: AnilistMedia | null) {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      if (media.idMal) {
        DEBUGS.JIKAN && console.log('[DEBUG] Using direct MAL ID:', media.idMal);

        const anime = await jikanApi.fetchAnimeById(media.idMal);
